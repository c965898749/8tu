...
function CheckReg_m() {
	...
	else if ($("#password_m").val().length < 6 || $("#password_m").val().length > 16) {
		alert("密码长度为6~16位字符！");
		$("#password_m").focus();
		return false;
	}
	else if (!ckupwd.test($("#password_m").val())) {
		alert("密码请使用[数字/字母/中划线/下划线]！");
		$("#password_m").focus();
		return false;
	}
	...
	else if ($("#password_m").val() != $("#repassword_m").val()) {
		alert("两次输入的密码不相同！");
		$("#repassword_m").focus();
		return false;
	}
...
function CheckEditInfo_m() 
{
	...
	else if ($("#username_m").val().length < 5 || $("#username_m").val().length > 32) {
		alert("用户名长度为5~32位字符！");
		$("#username_m").focus();
		return false;
	}
